...
mac - python3 varying_height_map.py <USERNAME> <PASSWORD> --map_dir <DIRECTORY> --mast_dir <MASTER_MP_DIR> --dist <DIST_IN_M> --end_n <END_N> --or_deg <0 or 180>
windows - python varying_height_map.py <USERNAME> <PASSWORD> --map_dir <DIRECTORY> --mast_dir <MASTER_MP_DIR> --dist <DIST_IN_M> --end_n <END_N> --or_deg <0 or 180>
...
from leo_funcs import check_batt_perc, convert_map_to_ply_3,control_height,nav_to_fid,fine_align,upload_map, log_test_metrics, ensure_recording_stopped,turn_relative
...
def main(argv):
    ...
    with LeaseKeepAlive(lease_client, must_acquire=False, return_at_exit=True):
        ...
        # Request all detected AprilTags from the persistent memory
        request_types = [world_object_pb2.WORLD_OBJECT_APRILTAG]
        world_objects = world_object_client.list_world_objects(object_type=request_types).world_objects
        ...
        for a in range(1, options.end_n + 1):
            ...
            convert_map_to_ply_3(full_path,ply_name,a,to_fiducial_matrix)
            end_time = time.time()
