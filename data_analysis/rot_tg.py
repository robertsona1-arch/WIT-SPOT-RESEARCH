...
def build_horizontal_panel(df_list, mode, panel_cfgs, save_path, global_title, colors, env_style_map):
    ...
    for idx, cfg in enumerate(panel_cfgs):
        ...
        if mode == 'primitives':
            # Plot individual lines for each primitive corner quadrant
            ...
            # Plot room-wide ensemble averages with spatial variance halos
            ...
            # Overlay both individual primitives and global trends on the same plot axis
        ...
        # Bottom placement ensures legends match standard formatting constraints
...
def main():
    parser = argparse.ArgumentParser(description="Compile Test 4 Rotational Multi-Panel Rows.")
    ...
    box_data = []
    print("Ingesting dataset rows for rotational scanning configurations...")
    ...
    if not box_data:
        print("[CRITICAL] Ingestion failure. Ensure standing_map_0deg and rotating_map files exist.")
        sys.exit(1)
    ...
    g_title = "Rotational Sensor Impact Analysis"

    print("\nCompiling rotational evaluation panels...")
    
    # PNG 1: Isolated Primitives
    build_horizontal_panel(combined_df, 'primitives', panel_cfgs, output_dir / "01_Primitives_Rotational_Panel.png", g_title, quad_colors, env_style_map)
    print("  [PANEL EXPORT] Saved 01_Primitives_Rotational_Panel.png")

    # PNG 2: Global averages with room variance bounds
    build_horizontal_panel(combined_df, 'global', panel_cfgs, output_dir / "02_Global_Average_Rotational_Panel.png", g_title, quad_colors, env_style_map)
    print("  [PANEL EXPORT] Saved 02_Global_Average_Rotational_Panel.png")

    # PNG 3: Composite Overlay
    build_horizontal_panel(combined_df, 'combined', panel_cfgs, output_dir / "03_Combined_Overlay_Rotational_Panel.png", g_title, quad_colors, env_style_map)
    print("  [PANEL EXPORT] Saved 03_Combined_Overlay_Rotational_Panel.png")

    print(f"\n[SUCCESS] Rotational plotting sequence complete. Output located in: /{output_dir.name}/")
