...
def main():
        ...
        try:
            ...
            for sheet_name in xls.sheet_names:
                ...
                if 'front' in sheet_lower and ('mid' not in sheet_lower and 'center' not in sheet_lower):
                    ...
                    # Calculate Derived Time/Snap Ratio Metrics
    ...
    plots_to_generate = [
        ...
        {'y': 'Volume_m3', 'x': 'Time', 'y_lbl': 'Volume (m³)', 'x_lbl': 'Time (s)', 'fn': 'Volume_vs_Time.png', 'is_vol': True},
        {'y': 'Percent_Error', 'x': 'Time', 'y_lbl': 'Volumetric Percent Error (%)', 'x_lbl': 'Time (s)', 'fn': 'PercentError_vs_Time.png', 'is_vol': False},
        
        # 10 Explicitly Inverted Axis Pairs (Y and X flipped)
        {'y': 'Point_Count', 'x': 'Snap_Count', 'y_lbl': 'Point Count (pts)', 'x_lbl': 'Snapshot Count', 'fn': 'PointCount_vs_SnapCount.png', 'is_vol': False},
        {'y': 'Density: Pts Per m3', 'x': 'Snap_Count', 'y_lbl': 'Spatial Density (pts/m³)', 'x_lbl': 'Snapshot Count', 'fn': 'Density_vs_SnapCount.png', 'is_vol': False},
        {'y': 'Point_Count', 'x': 'Time', 'y_lbl': 'Point Count (pts)', 'x_lbl': 'Time (s)', 'fn': 'PointCount_vs_Time.png', 'is_vol': False},
        {'y': 'Density: Pts Per m3', 'x': 'Time', 'y_lbl': 'Spatial Density (pts/m³)', 'x_lbl': 'Time (s)', 'fn': 'Density_vs_Time.png', 'is_vol': False},
        {'y': 'Snap_Count', 'x': 'Time', 'y_lbl': 'Snapshot Count', 'x_lbl': 'Time (s)', 'fn': 'SnapCount_vs_Time.png', 'is_vol': False},
        {'y': 'Density: Pts Per m3', 'x': 'Point_Count', 'y_lbl': 'Spatial Density (pts/m³)', 'x_lbl': 'Point Count (pts)', 'fn': 'Density_vs_PointCount.png', 'is_vol': False},
        {'y': 'Points_Per_Snap', 'x': 'Snap_Count', 'y_lbl': 'Points per Snapshot (pts/snap)', 'x_lbl': 'Snapshot Count', 'fn': 'PointsPerSnap_vs_SnapCount.png', 'is_vol': False},
        {'y': 'Avg_Points_Per_Time', 'x': 'Time', 'y_lbl': 'Average Points per Time (pts/s)', 'x_lbl': 'Time (s)', 'fn': 'AvgPointsPerTime_vs_Time.png', 'is_vol': False},
        {'y': 'Density_Per_Time', 'x': 'Time', 'y_lbl': 'Density Accumulation Rate (pts/m³/s)', 'x_lbl': 'Time (s)', 'fn': 'DensityPerTime_vs_Time.png', 'is_vol': False},
        {'y': 'Density_Per_Snap', 'x': 'Snap_Count', 'y_lbl': 'Density per Snapshot (pts/m³/snap)', 'x_lbl': 'Snapshot Count', 'fn': 'DensityPerSnap_vs_SnapCount.png', 'is_vol': False}
    ]
    ...
    for plot_cfg in plots_to_generate:
        ...
        fig, ax = plt.subplots(figsize=(9, 5.8)) # Slightly narrowed width since legend is inside canvas
        ...
        for (cond, prim), group_data in master_df.groupby(['Operational_Condition', 'Primitive_Target']):
            ...
            # Conditionally inject physical ground truth value if handling the volume graph
            if plot_cfg['is_vol']:
                label_string = f"{prim} ({cond.split(' (')[0]}) [Actual: 0.02286 m³]"
            else:
                label_string = f"{prim} ({cond.split(' (')[0]})"
        ...
        # FIXED: Legend moved inside graph bounds; searches for best open quadrant automatically
        ...
        plt.close()
        print(f"  [SAVED PNG] -> {plot_cfg['fn']}")

    print("\nGlobal data plotting sequence finalized. In-graph legend optimization complete.")
