...
def main():
    parser = argparse.ArgumentParser(description="Compare specific objects across standard operating environments.")
    parser.add_argument('--folder', required=True, help="Root directory containing the test condition subfolders")
    parser.add_argument('--name', default="Cross-Condition", help="Official Test Name to replace {test name} in titles")
    ...
    # Core experimental folder names (standing_map_180deg removed)
    target_subfolders = [
        'closer',
        'rotating_map',
        'standing_map_0deg',
    ...
    if not valid_paths:
        print("[CRITICAL] None of the specified subfolders were located. Check path structure.")
        sys.exit(1)
    ...
    print(f"\nGenerating standardized object graphs into: {output_root_dir.name}/")
    ...
    for obj_name, df_list in object_master_data.items():
        combined_obj_df = pd.concat(df_list, ignore_index=True)
        ...
        # Dynamically build the line map based on the environments present for this specific object
        env_combos = sorted(combined_obj_df['Test_Environment'].unique())
        env_style_map = {}
        for i, env in enumerate(env_combos):
            env_style_map[env] = {'color': colors[i % len(colors)], 'marker': markers[i % len(markers)], 'linestyle': linestyles[i % len(linestyles)]}
        ...
        for idx, plot_cfg in enumerate(plots_to_generate, 1):
            ...
            # Format title to dynamically show the object name
            dynamic_test_name = f"{test_name} ({obj_name})"
            final_title_base = plot_cfg['title'].replace('{test name}', dynamic_test_name)

            if x_col in combined_obj_df.columns and y_col in combined_obj_df.columns:
                fig, ax = plt.subplots(figsize=(10, 6.5))
                
                for env, group_df in combined_obj_df.groupby('Test_Environment'):
                    ...
                    style = env_style_map.get(env)
                    ax.plot(sorted_df[x_col], sorted_df[y_col], color=style['color'], linestyle=style['linestyle'],
                            marker=style['marker'], linewidth=2.5, markersize=7, alpha=0.9, label=label_str)
                ...
                plt.tight_layout()
                plt.savefig(obj_plot_dir / f"{idx:02d}_{safe_title}.png", dpi=300, bbox_inches='tight')
                plt.close()
            else:
                missing = [c for c in [x_col, y_col] if c not in combined_obj_df.columns]
                print(f"    [SKIP] Plot '{safe_title}': Missing target columns {missing}")

    print(f"\n[SUCCESS] Cross-test object execution complete.")
