...
def main():
    parser = argparse.ArgumentParser(description="Compare standing 0 deg vs varying height 0 deg conditions for quadrants and run averages.")
    ...
    # Isolate target tests at 0 degrees
    ...
    if len(valid_paths) < 2:
        print("[CRITICAL] Height impact analysis requires BOTH standing_map_0deg and varying_height_0deg folders.")
        sys.exit(1)
    ...
    print("Extracting sheets from 0-degree test environments...")
    ...
    # Structure separate output directories to maintain a clean workspace
    parent_output_dir = base_dir / "Height_Impact_Analysis_0deg"
    ...
    # Strict IEEE Color Schemes (Solid colors, no transparency layers)
    quad_color_map = {
        ('Front Left Box', 'standing_map_0deg'): '#d62728',     # Solid Red
        ('Front Left Box', 'varying_height_0deg'): '#ff9896',   # Light Red
        ('Front Right Box', 'standing_map_0deg'): '#ff7f0e',    # Solid Orange
        ('Front Right Box', 'varying_height_0deg'): '#fdbf6f',   # Light Orange
        ('Back Left Box', 'standing_map_0deg'): '#1f77b4',      # Solid Blue
        ('Back Left Box', 'varying_height_0deg'): '#a6cee3',     # Light Blue
        ('Back Right Box', 'standing_map_0deg'): '#2ca02c',     # Solid Green
        ('Back Right Box', 'varying_height_0deg'): '#b2df8a'    # Light Green
    }

    avg_color_map = {
        'standing_map_0deg': '#000000',    # Solid Black for baseline
        'varying_height_0deg': '#d62728'   # Solid Red for variation
    }
    ...
    for idx, (x_col, y_col, title) in enumerate(metric_pairs, 1):
        ...
        for (quadrant, env), group_df in combined_quad_df.groupby(['Quadrant_Class', 'Test_Environment']):
            ...
            env_label = "Fixed H" if env == 'standing_map_0deg' else "Varying H"
        ...
        ax.set_title(f"Height Impact: Quadrant Comparison (0° Heading)\n{title}", pad=12)
        ax.set_xlabel(x_col.replace('_', ' '))
        ax.set_ylabel(y_col.replace('_', ' '))
        ax.legend(loc='best', framealpha=1.0, edgecolor='black')
        plt.tight_layout()
        plt.savefig(quad_output_dir / f"Quad_0deg_{idx:02d}_{safe_title}.png", dpi=300)
        plt.close()
        ...
        for env, group_df in combined_dash_df.groupby('Test_Environment'):
            sorted_df = group_df.sort_values(by=x_col)
            line_style = '-' if env == 'standing_map_0deg' else '--'
            env_label = "Fixed Height Baseline" if env == 'standing_map_0deg' else "Varying Height Profile"
        ...
        ax.set_title(f"Global Run Averages Comparison (0° Heading)\n{title}", pad=12)
        ax.set_xlabel(x_col.replace('_', ' '))
        ax.set_ylabel(y_col.replace('_', ' '))
        ax.legend(loc='best', framealpha=1.0, edgecolor='black')
        plt.tight_layout()
        plt.savefig(avg_output_dir / f"Avg_0deg_{idx:02d}_{safe_title}.png", dpi=300)
        plt.close()

    print(f"\n[SUCCESS] Height analysis at 0 degrees complete.")
