...
def main():
    parser = argparse.ArgumentParser(description="Aggregate Spot active stereo test averages per object and summary dashboards including error metrics.")
    ...
    for test_type, sheets_dict in test_groups.items():
        ...
        # Create a tracker to build the macro dashboard metrics dynamically
        running_abs_errors = []
        running_pct_errors = []
        ...
        with pandas.ExcelWriter(output_filename, engine='openpyxl') as writer:
            ...
            for sheet_name in sorted(sheets_dict.keys(), key=lambda x: x == 'Averages_Dashboard'):
                ...
                if sheet_name == 'Averages_Dashboard':
                    # First, run standard mean aggregation on the original dashboard metrics
                    averaged_df = combined_df.groupby('Snap_Count').mean(numeric_only=True).reset_index()
                    
                    # Intercept and append macro global error trends if object data was available
                    if running_abs_errors and running_pct_errors:
                        # Combine errors across all primitives to get macro vectors aligned by row count
                        all_abs_df = pandas.concat(running_abs_errors, axis=1)
                        all_pct_df = pandas.concat(running_pct_errors, axis=1)
                        
                        averaged_df['Global_Mean_Absolute_Error_m3'] = round(all_abs_df.mean(axis=1), 6)
                        averaged_df['Global_Mean_Percent_Error'] = round(all_pct_df.mean(axis=1), 2)
                        print(f"  [DASHBOARD INTEGRATION] Appended macro global error averages to tracking matrix.")
                    
                    averaged_df.to_excel(writer, index=False, sheet_name=sheet_name)
                else:
                    # Compute mean tracking profiles for primitive target boxes
                    averaged_df = combined_df.groupby('Snap_Count').mean(numeric_only=True).reset_index()
                    
                    # Cache the error columns row-wise to integrate them into the master dashboard later
                    if 'Absolute_Error_m3' in averaged_df.columns:
                        running_abs_errors.append(averaged_df['Absolute_Error_m3'])
                    if 'Percent_Error' in averaged_df.columns:
                        running_pct_errors.append(averaged_df['Percent_Error'])
