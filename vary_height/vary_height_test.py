...
python vary_height_test.py <USERNAME> <PASSWORD> --end_n <END_N>

This is a test script for the vary_height.py script, designed to validate the scripts function without accessing the robot hardware that the vary_height script runs on.
...
def main(argv):
    ...
    # No Mapping data should be created, so no directory needed
    ...
    with LeaseKeepAlive(lease_client, must_acquire=False, return_at_exit=True):
        ...
        for a in range(1, options.end_n + 1):
            ...
            # No Mapping data should be created, so no directory recording
            ...
            for b in range(a):
                if a!=1:
                    # No Mapping data should be created, so no recording
                    # #snapshot
...
# May get rid of this function because it is not used for vary_height
...
# Same problem as varying_height_map where SE3Trajectory needs a different set of arguments that SE3Pose:
#I don't know the proper set of values needed